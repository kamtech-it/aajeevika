...
class CategoryController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->has('s')) {
            ...
            ->select('categories.*', 'users.name as admin_name')
            ->where('parent_id', '=', '0')
            ->where('categories.name_en', 'LIKE', '%' . Input::get('s') . '%')
            ->orWhere('categories.name_kn', 'LIKE', '%' . Input::get('s') . '%')
            ->orderBy('id','desc');
        }
    ...
    {
        // echo "Cat Id : ".$catId; die("check");
        // $categoryData = DB::table('categories')->get();
        $categoryData = Category::where(['is_active' => 1, 'parent_id' => 0])->get();
        return view('category.add', ['categoryData' => $categoryData,]);
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($files = $request->file('image')) {
                ...
                $aa =  $files->move(public_path("images/category"), $profileImage);
                $input['image'] = "images/category/".$profileImage;
            }
            ...
            return redirect('admin/category')->with('message', $queryStatus);
        }
    }
    public function edit($id)
    {
        ...
        return view('category.edit', ['categoryDetail' => $categoryDetail, 'categoryData'=>$categoryData, 'id' => $id,]);
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($validator->fails()) {
            return redirect('admin/editCategory/'.$id)->withErrors($validator)->withInput();
        } else {
            $input = $request->all();
            $input['slug'] = Str::slug($request->name_en);
            if ($files = $request->file('image')) {
    ...
    {
        $categoryData = Category::with('subcategory')->where(['id' => $id])->first();
        // dd($categoryData);
    ...
    {
        $catStatus = Category::where('id', $id)->first();
